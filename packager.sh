...
#    add_custom_pkg_repo
    ...
    ssh -t "$lxc_ip" "
        ...
        debuild --set-envvar GEM_SET_DEBUG=\$GEM_SET_DEBUG  -i -b
        # here the code
...
EOF
    ...
    if [ $BUILD_REPOSITORY -eq 1 -a -d "${GEM_DEB_REPO}" ]; then
        if [ "$branch_id" != "" ]; then
            repo_id="$(repo_id_get)"
            if [ "$repo_id" != "$GEM_GIT_REPO" -o "$branch_id" != "$GEM_MASTER_BRANCH" ]; then
        ...
        # if the monotone directory exists and is the "gem" repo and is the "master" branch then ...
        if [ -d "${GEM_DEB_MONOTONE}/${BUILD_UBUVER}/binary" ]; then
            if [ "$repo_id" == "$GEM_GIT_REPO" -a "$branch_id" == "$GEM_MASTER_BRANCH" ]; then
